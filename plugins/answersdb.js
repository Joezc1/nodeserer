...
module.exports = {
    find: function (answer,x, y, params, callback) {
        ...
        let sql =""
        console.log(answer)
        sql = 'select * from answers where topicid like "%'+answer.topicid+'%" limit '+x+","+y;
        console.log('打印查询语句')
        console.log(sql)
    ...
    deleteById: function (id, params, callback) {
     ...
     //根据用户id数组循环获取用户信息
     findsByAnswerid: function (list, params, callback) {
        //每次使用的时候需要创建链接，数据操作完成之后要关闭连接
        var connection = mysql.createConnection(data);
        let userlist = []
        connection.connect(function (err) {
            ...
            for (let i = 0; i < list.length; i++) {
                let sql = "select * from answers where id=" + "'" + list[i].answerid + "'"
                //开始数据操作
                console.log(sql)
                connection.query(sql, params, function (err, results, fields) {
                    if (err) {
    ...
    updateById: function (answer,id, params, callback) {
        //每次使用的时候需要创建链接，数据操作完成之后要关闭连接
        let sql = "update answers set abstract='" + answer.abstract + "',agree=" + "'" + answer.agree + "',createtime=" + "'" +answer.createtime + "',comments=" + "'" + answer.comments + "',answer=" + "'" + answer.answer +"',topicid=" + "'" + answer.topicid +"',userid=" + "'" + answer.userid + "' where id =" + id
        console.log("修改公告")
        console.log(sql)
        var connection = mysql.createConnection(data);
        connection.connect(function (err) {
