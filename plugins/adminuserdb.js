...
module.exports = {
    ...
    updateById: function (user, id, params, callback) {
        ...
        //每次使用的时候需要创建链接，数据操作完成之后要关闭连接
        let sql = "update adminuser set username='" + user.username + "',password=" + "'" + user.password +"',login=" + "'" + user.login+ "',level=" + "'" + user.level + "',email=" + "'" + user.email + "',tel=" + "'" + user.tel + "' where id =" + id
        console.log("修改用户")
        console.log(sql)
        var connection = mysql.createConnection(data);
        connection.connect(function (err) {
